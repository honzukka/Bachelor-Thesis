...
After a difference metric is computed, it is placed as a 3D vector in the corresponding vertex of the {\it rendered mesh}. For a better understanding, here is the meaning of such a representation in case of both chosen metrics:
...
\stepcounter{footnote}\footnotetext{Hierarchical clustering, also called bottom-up clustering, starts with each data point representing an elementary cluster. In each step of the algorithm, two clustering candidates are found from the available clusters according to certain criteria and subsequently merged. Hierarchical clustering creates a binary tree, also called a dendrogram, where each node represents a cluster and has two children from which it was created. Arbitrary number of clusters covering the whole data set can then be obtained by taking roots of disjoint subtrees which, when combined, contain all the leaves of the dendrogram. See Fig. \ref{fig:illustration-telea_hierarchical_clustering}.}
...
\begin{figure}[h]
	...
	\begin{subfigure}{0.5\textwidth}
    	...
    	\caption{Various clusterings of a data set and the associated dendrogram. Clusters corresponding to the original data points are gray.}
	\end{subfigure}
    \qquad
...
\centering
    ...
    \caption{Arrows which represent excluded clusters are missing}
