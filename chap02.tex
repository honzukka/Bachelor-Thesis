...
\caption[Corner Table Illustration]{An illustartion of the corner table. The black corner has direct access to all red elements - its vertex and certain surrounding corners}
...
The main public method of \verb+DiffVector+ is \verb+CreateVisualization()+. It is mainly responsible for metric computation and data point clustering. All this computed data is stored inside \verb+DiffVector+ instances. When the clustering is ready, a visualization is created and outputted from \verb+CreateVisualization()+ using a visualizer object.
...
The inverted visualization is obtained by multiplying the representative vector by -1 and reassigning the arrow {\it scene} colors according to this version of the vector.
...
The inverted visualization is also obtained by multiplying the representative vector by -1 and reassigning the arrow {\it scene} colors according to this version of the vector.
...
\item For the case when arrows are part of a visualization, we have added the option to toggle wireframe view independently for them and the underlying {\it scene}. The arrows therefore need to be stored in a separate {\it scene} and the option to render two {\it scenes} at once had to be added to the rendering methods. In the visualization process, this is achieved by baking the color visualization into the underlying {\it scene} and the arrow visualization into an empty {\it scene} (see section \ref{sec:implementation_visualizers}).
\item Lastly, the user is able to switch between viewing the computed visualizations and the raw triangle meshes. Visualization {\it scenes} (original with colors and a separate one with arrows) and raw {\it scenes} (only original ones) are therefore stored separately for these actions to happen quickly. Also, rendering methods now are now more flexible to make the transition between rendering various {\it scenes} easier.
\end{itemize}

For an illustration of the last two points, see Fig. \ref{fig:scenes_explained}.
...
The are two \verb+DiffVector+ instances to support different clustering for arrow visualization and color visualization as described in section \ref{implemetation-user_interface}.
